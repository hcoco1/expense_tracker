...
/**
 * Build a human-readable share summary for the current filter period.
 */
export function buildShareText(expenses, categories, filters) {
  ...
  let text = `📊 Expense Tracker — ${label}\n`
  text += `💰 Balance: ${formatCurrency(summary.balance, filters.currency)}\n`
  text += `📈 Income: ${formatCurrency(summary.income, filters.currency)}\n`
  text += `📉 Expenses: ${formatCurrency(summary.expense, filters.currency)}\n`
  if (rate > 0) text += `💾 Savings rate: ${rate}%\n`
  if (top.length) {
    text += `\nTop spending:\n`
...
/**
 * Invoke the native Web Share API.
 * Returns true if the share sheet was triggered, false if API unavailable.
 */
